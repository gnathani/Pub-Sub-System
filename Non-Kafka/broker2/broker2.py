...
broker2 = Flask(__name__)
broker2.secret_key = 'adgn2'

def get_my_topics():
    teams_list = ["Houston Rockets","Indiana Pacers","LA Clippers","Los Angeles Lakers","Memphis Grizzlies","Miami Heat","Milwaukee Bucks","Minnesota Timberwolves","New Orleans Pelicans","New York Knicks"]
    return teams_list

def get_my_neighbour():
    return 'broker_server3'

@broker2.route('/', methods=['GET', 'POST'])
def login():
...
@broker2.route('/home', methods=['GET','POST'])
def home():
        ...
        if request.method == 'POST':
            ...
            if new_sub not in t_list:
                ...
                if new_sub not in my_teams: 
                    ...
                    r = requests.post("http://"+my_nbr+":5002/sub",json=to_send)
...
@broker2.route('/register', methods=['GET', 'POST'])
def register():
...
@broker2.route('/profile', methods=['GET','POST'])
def profile():
    form = Form1()
    client = MongoClient(host='test_mongodb', port=27017, username='root', password='pass', authSource="admin")
    db = client["user_db"]
...
@broker2.route('/pub',methods=['POST'])
def return_message2():
    data = request.json
    publish_news(data)
...
def publish_news(news_all):
    ...
    if to_send != []: r = requests.post("http://"+my_nbr+":5002/pub",json=to_send)
    return 0

@broker2.route('/sub',methods=['POST'])
def subscribe():
    my_teams = get_my_topics()
    my_nbr = get_my_neighbour()
    client = MongoClient(host='test_mongodb', port=27017, username='root', password='pass', authSource="admin")
    db = client["user_db"]
    data=request.json
    if data[1] not in my_teams: r = requests.post("http://"+my_nbr+":5002/sub",json=data)
    else: db.subscr_tb.insert_one({"username": data[0], "topic": data[1]})
    return "sub done"

if __name__=='__main__':
    broker2.run(host="0.0.0.0", port=5001)